...
/**
 * This class contains the methods and attributes used by library staff. to
 * manage the users, resources and the borrowing and returning of resources.
 * 
...
public class Librarian extends User {
	 ...
	 * 
	 * @param staffNo
	 *            The staff number of the librarian.
	 * @param employmentDate
	 *            The date the librarian started working.
	 * @param username
	 ...
	 * @param mobileNo
	 *            Mobile number of the librarian
	 * @param address
	 *            Address of the librarian.
	 * @param userImage
	 *            Photo of the librarian.
	 */
	public Librarian(int staffNo, int employmentDate, String username, String password, String firstName,
			String lastName, int mobileNo, String address, int userImage) {
	 ...
	 * 
	 * @param employment
	 *            date The date the librarian started working.
	 * 
	 */
	 ...
	 * @param year
	 *            The year the resource was released.
	 * @param image
	 *            An image of the resource.
	 * @param numCopies
	 *            The number of copies of the resource owned by the library.
	 * @param duration
	 ...
	 *            The number of copies available to borrow from the library.
	 * @param duration
	 *            The duration of the DVD.
	 * @param director
	...
	public void addDvd(String title, String year, String image, int numAvCopies, int duration, String director,
		...
		// then add them to the DVD_subtitle table along with the corresponding
		// resourceID.
	...
	/**
	 * Show each borrowing event associated with the item, including date borrowed,
	 * date returned, borrowing user.
	 * 
	 * @param resourceID
	...
	public String getBorrowingHistory(int resourceID) throws SQLException {
		// SQL query to get date borrowed, date returned, borrowing user from the
		// database.
		...
		while (r.next()) {
			result += String.format("%20s %20s %20s \n", r.getString("borrowDate"), r.getString("returnDate"),
	 ...
	 * @throws SQLException
	 */
	public void addLaptop(String title, String year, String image, int numAvCopies, int duration, String manufacturer,
			String model, String operatingSystem) throws SQLException {
		int id = addResource(title, year, image, numAvCopies, duration);
	 ...
	 * @throws SQLException
	 */
	public void editResource(int resourceId, String title, String year, String image, int numAvCopies, int duration)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public void editBook(int resourceId, String title, String year, String image, int numAvCopies, int duration,
	 ...
	 * @throws SQLException
	 */
	public void editDVD(int resourceId, String title, String year, String image, int numAvCopies, int duration,
		...
		if (subtitle.length() != 0) {
			...
			for (int i = 0; i < subtitleLang.size(); i++) {
				String subLang = subtitleLang.get(i);
				String query2 = "insert into DVD_subtitle (resourceID, subtitle)" + "values('" + resourceId + "','"
						+ subLang + "');";
				sql.set(query2);
			}
		}
	 ...
	 * @param userName
	 *            The name of the user borrowing the resource.
	 * @throws Exception
	 */
	 ...
	 * 
	 * @param borrowNo
	 *            The identifier of the borrowing event.
	 */
	public void returnResource(int borrowNo) throws Exception {
		...
		// If the resource is overdue, it reduces the user's balance by the calculated
		// amount.
		if (b.isOverdue()) {
			...
			r = sql.nonStaticGet(statement);
			int i = 0;
			// Finds the max transID and increases it by one to ensure unique transaction
			// ID.
			while (r.next()) {
			...
			statement = "INSERT INTO `transaction`(`transID`, `amount`, `date`) VALUES ('" + i + "','" + b.fine()
					+ "','" + getCurrentTime() + "')";
			sql.set(statement);
	 ...
	 * @param username
	 *            The name of the user paying the fine.
	 */
	...
	/**
	 * This method allows a librarian to create a new User. Hashcode is stored in
	 * the password field.
	 * 
	 * @param username
	...
	public void newLibrarian(String username, String password, String firstname, String lastname, int mobileNo,
		...
		// Get new unique StaffNo by increasing maximum staffNo by 1, ensuring unique
		// ID.
